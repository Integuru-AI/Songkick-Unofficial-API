...
class SongkickIntegration(Integration):
    ...
    async def _handle_response(self, response: aiohttp.ClientResponse):
        ...
        try:
            ...
            response_json = {"error": {"message": "Unknown error", "code": str(response.status)}}
    ...
    async def search_location(self, location_name:str):
        ...
        locations_response = self.session.get(get_locations_url, headers={
            "Cookie": self.cookies
        })
        ...
        if "Sorry, we found no results for" in locations_response.text:
            ...
            raise HTTPException(
                status_code = 404,
                detail = f"No results found for {location_name}"
            )
        ...
        for li in soup.find_all("li", class_="small-city"):
            ...
            track_form = li.find("form", {"data-analytics-category": "track_metro_area_button"})
            if track_form:
                track_url = "https://www.songkick.com" + track_form["action"]
                authenticity_token = track_form.find("input", {"name": "authenticity_token"})["value"]
                relationship_type = track_form.find("input", {"name": "relationship_type"})["value"]
                subject_type = track_form.find("input", {"name": "subject_type"})["value"]
                success_url = track_form.find("input", {"name": "success_url"})["value"]
            else:
                track_url = authenticity_token = relationship_type = subject_type = success_url = None
    ...
    async def track_untrack_location(self, request: TrackUntrackLocationRequest):
        ...
        logger.debug(f"Attempting to track/untrack location with id: {request.subject_id}")
        ...
        trackings_response = self.session.post(trackings_url, data=trackings_form, headers={
            "Cookie": self.cookies,
        })
        try:
            trackings_response.raise_for_status()
            if trackings_response.status_code == 200 and not '"status":"ok"' in trackings_response.text:
                return {"status": "ok"}
            ...
            logger.debug(f"An error occured while attempting to track location({request.subject_id}):  {str(exc)}")
            return {"status": "failed"}
        
    async def get_events(self):
        logger.debug("Fetching all events for user")
        get_tracked_artists_url = "https://www.songkick.com/calendar?filter=tracked_artist"
        get_tracked_artists_response = self.session.get(get_tracked_artists_url, headers={
            "Cookie": self.cookies,
        })
        get_tracked_artists_response.raise_for_status()

        if (get_tracked_artists_response.status_code != 200 and 
            not "All upcoming concerts from the artists you’re tracking." 
        ...
        for event in event_listings:
            ...
            venue_name = venue_tag.find("span", class_="venue-name").text.strip() if venue_tag else None
            city_state = venue_tag.find_all("span")[1].text.strip() if venue_tag else None
            street_address = venue_tag.find("span", class_="street-address").text.strip() if venue_tag and venue_tag.find("span", class_="street-address") else None
            event_url = event.find("a")["href"] if event.find("a") else None
            image_url = event.find("img")["src"] if event.find("img") else None
            ticket_url = event.find("a", class_="button buy-tickets")["href"] if event.find("a", class_="button buy-tickets") else None
            ...
            events.append({
                ...
                "event_url": f"https://www.songkick.com{event_url}" if event_url else None,
                "image_url": f"https:{image_url}" if image_url else None,
                "ticket_url": f"https://www.songkick.com{ticket_url}" if ticket_url else None
            })
    ...
    def get_event_details(self, event_url):
        ...
        get_event_response = self.session.get(event_url, headers={
            "Cookie": self.cookies
        })
        ...
        date_time = soup.select_one(".date-and-name p").text.strip() if soup.select_one(".date-and-name p") else None
        name = soup.select_one(".summary a").text.strip() if soup.select_one(".summary a") else None
        
        location = (
            soup.select_one(".location .name a").text.strip() + ", " +
            soup.select_one(".location span a").text.strip()
        ) if soup.select_one(".location .name a") and soup.select_one(".location span a") else None
        
        image_url = soup.select_one(".profile-picture-wrapper img").get("src") if soup.select_one(".profile-picture-wrapper img") else None
        
        tickets = []
        for ticket in soup.select(".buy-ticket-link"):
            vendor = ticket.select_one(".vendor").text.strip() if ticket.select_one(".vendor") else "Unknown Vendor"
            link = ticket.get("href")
            tickets.append({"vendor": vendor, "link": f"https://www.songkick.com{link}"})
        
        venue_name = soup.select_one(".venue-info-details a").text.strip() if soup.select_one(".venue-info-details a") else None
        venue_address = (
            soup.select_one(".venue-hcard span").text.strip() if soup.select_one(".venue-hcard span") else None
        )
        ...
        if additional_container:
            ...
            if "Price:" in details_text:
                additional_details["price"] = details_text.split("Price: ")[1].split(" ")[0]
        ...
        event_return = {
            ...
            "name": name,
            "location": location,
            "image_url": f"https:{image_url.replace('medium_avatar', 'huge_avatar')}" if image_url else None,
